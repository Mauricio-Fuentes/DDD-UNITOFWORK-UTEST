...
export class ConsignBankAccountService{
  ...
  async execute(request: ConsignBankAccountRequest) : Promise<ConsignBankAccountResponse>{
    ...
    if(accountOrm != undefined){
      ...
      //return new ConsignBankAccountResponse('Se consignaron ' + request.value + ' a la cuenta: ' + bankAccount.number + ' balance total: ' + bankAccount.balance);
      return new ConsignBankAccountResponse('Se consignaron ' + request.value + ' a la cuenta: ' + bankAccount.number);
    }
